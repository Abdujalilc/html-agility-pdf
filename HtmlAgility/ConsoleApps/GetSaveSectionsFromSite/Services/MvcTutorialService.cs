namespace Services
{
    ...
    public interface IMvcTutorialService
    {
        Task GetMvcTutorials(string mainUrl, string? outputDir = null);
    }

    public class MvcTutorialService : IMvcTutorialService
    {
        private readonly HttpClient _httpClient;
        public MvcTutorialService(HttpClient httpClient)
        {
            _httpClient = httpClient;
            _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("Mozilla/5.0 (compatible; AcmeInc/1.0)");
        }

        public async Task GetMvcTutorials(string mainUrl, string? outputDir = null)
        {
